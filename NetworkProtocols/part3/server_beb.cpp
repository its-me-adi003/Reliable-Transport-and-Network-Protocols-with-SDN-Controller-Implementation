            ...
            if (offset >= m) {
                ...
                while (start < t) {
                    ...
                    if ((server_state->active_clients>1) or (does_collide)){
                        ...
                        server_state->collide=false;
                        does_collide=true;
                        auto time_now = new chrono::time_point<chrono::system_clock>(chrono::system_clock::now());
                        server_state->last_concurrent_time.compare_exchange_strong(last_time, time_now);
                        server_state->socket_id=-1;
                        break;
                    }
                    message_to_send = "";
