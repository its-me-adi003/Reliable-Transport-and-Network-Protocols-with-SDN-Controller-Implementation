...
struct server_data {
    vector<queue<pair<client_data*, string>>> client_queues; // One queue per client socket
    int rem;  // Number of remaining client connections to be served
    int current_client;  // Keep track of which client's queue we are currently serving

    server_data(int n) : rem(n), current_client(0) {
        client_queues.resize(n);  // Initialize n queues for n clients
    }
};
...
struct client_data {
    ...
    bool is_done = false;
    int client_id;
    int socket_id;  // To identify socket (1 to 5)
...
void* queue_solver(void* arg) {
    ...
    while (true) {
        ...
        queue_cv.wait(lock, [&server]() {
            for (auto& q : server->client_queues) {
                if (!q.empty()) return true;  // Wake up if at least one queue has data
            }
            return false;
        });

        bool served = false;
        for (int i = 0; i < server->client_queues.size(); i++) {
            int client_idx = (server->current_client + i) % server->client_queues.size();
            if (!server->client_queues[client_idx].empty()) {
                pair<client_data*, string> p1 = server->client_queues[client_idx].front();
                server->client_queues[client_idx].pop();
                ...
                // Parse the offset from the message
                while (message[j] != '\n') {
                    offset = offset * 10 + (message[j] - '0');
                    j++;
                }
                cout << "Client ID: " << data->client_id << ", Socket ID: " << data->socket_id << " | Offset: " << offset << endl;
                ...
                if (offset >= m) {
                    ...
                    if (offset + k >= m) {
                        cout << "Final packet will be sent" << endl;
                        is_this_last_offset = true;
                    }

                    // Send the data in packets of size 'p'
                    ...
                    while (start < t) {
                        ...
                        if (start >= t && is_this_last_offset) {
                            message_to_send += EOF;  // EOF logic as the end marker for the last packet
                        }
                        message_to_send += "\n";
                    ...
                    // If this was the last offset, mark the client as done
                ...
                server->current_client = (client_idx + 1) % server->client_queues.size();
                served = true;
                break;
            }
        }
        ...
        if (!served) {
            queue_cv.notify_one();  // Re-notify in case no queue had data
        }
    }
}
...
void* handle_client(void* arg) {
    ...
    while (true) {
        if (data->is_done) {
            return nullptr;  // Exit when the client is done
        } else {
            char message[1024] = {0};
...

            {
                ...
                // data->server_state->client_queues[data->client_id * 5 + data->socket_id].push({data, temp_message});
                data->server_state->client_queues[data->client_id].push({data, temp_message});
            }
...
int main() {
    ...
    listen(server_socket, n + 4);  // Adjusted for 5 sockets per client

    cout << "Waiting for connections..." << endl;

    server_data server_state(n );
    ...
    while (true) {
        ...
        // client_data* data = new client_data(&server_state, client_count / 5, client_count % 5);  // Allocate memory for the client data
