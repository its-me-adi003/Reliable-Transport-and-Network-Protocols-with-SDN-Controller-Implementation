...
struct server_data {
    vector<queue<pair<client_data*, string>>> client_queues; // One queue per client
...
struct client_data {
    ...
    client_data(server_data* state,int id) : server_state(state), client_id(id){}  // Constructor to initialize server_state
};
...
void* queue_solver(void* arg) {
    ...
    while (true) {
        ...
        for(int i=0;i< server->client_queues.size();i++){
            ...
            if (!server->client_queues[client_idx].empty()) {
                ...
                if (offset >= m) {
                    ...
                    if (offset + k >= m) {
                        cout<<"final packet will be send"<<endl;
                ...
                lock.lock();
                server->current_client = (client_idx + 1) % server->client_queues.size();  // Move to the next client in RR fashion
                served = true;
                break;
            }
        }
        ...
        if (!served) {
            queue_cv.notify_one();  // Re-notify in case no queue had data, which is unlikely
        }        
...
int main() {
    ...
    while (true) {
        ...
        client_data* data = new client_data(&server_state,client_count);  // Allocate memory for the client data
