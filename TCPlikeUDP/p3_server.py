 ...
 #   print(WINDOW_SIZE)
  #  return None 
    ...
    with open(file_path, 'rb') as file:
        ...
        W_max=0 
        B=0.5 
        is_cubic_vaild=False 
        congestion_start_time=0
    
        while True:
         #   print(time.time()-throughput_time,last_ack_received,ack_throughput)
            if time.time()-throughput_time>=1:
                ...
                throughput_time=time.time()
                ack_throughput=last_ack_received
            if congestion_avoidance and is_cubic_vaild:
                cwnd= C*((time.time()-congestion_start_time-K)**2)+W_max
       #     print("cwnd :",cwnd)
...
def retransmit_unacked_packets(server_socket, client_address, unacked_packets,pack_to_retransmit):
  ...
  #  print(earliest_seq_num in unacked_packets,earliest_seq_num)
...
def fast_recovery(server_socket, client_address, unacked_packets,ack_seq_num):
  ...
  #  print(earliest_seq_num in unacked_packets)
    server_socket.sendto(unacked_packets[earliest_seq_num][0], client_address)
